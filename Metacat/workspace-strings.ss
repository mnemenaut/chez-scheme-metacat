	...
	(let ((self (1st msg)))
	  (record-case (rest msg)
	    ...
	    (get-equivalent-bond (bond)
	      (let* ((i (tell (tell bond 'get-from-object) 'get-id-number))
		     (j (tell (tell bond 'get-to-object) 'get-id-number))
		...
		(if (and (exists? equivalent-flipped-bond)
		  ...
		  #f)))
	    (assign-id-number (object)
	      (tell object 'set-id-number next-id-number)
	      (set! next-id-number (add1 next-id-number))
	      (if* (= next-id-number max-object-capacity)
		(tell self 'expand-internal-storage))
	      'done)
	    (add-letter (letter position)
	      (tell self 'assign-id-number letter)
	    ...
	    (set-letter-list ()
	      ...
	      'done)
	    (add-bond (bond)
	      (let ((i1 (tell (tell bond 'get-from-object) 'get-id-number))
		    (j1 (tell (tell bond 'get-to-object) 'get-id-number))
		    (i2 (tell (tell bond 'get-left-object) 'get-id-number))
		    (j2 (tell (tell bond 'get-right-object) 'get-id-number)))
	    ...
	    (delete-proposed-bonds (object)
	      (let ((i (tell object 'get-id-number)))
		...
		group
		(let ((i (tell (tell group 'get-leftmost-object) 'get-id-number)))
	    ...
	    (add-group (group)
	      (tell self 'assign-id-number group)
	      (let* ((left-pos (tell group 'get-left-string-pos))
		     ...
		     (i (tell (tell group 'get-leftmost-object) 'get-id-number)))
	    ...
	    (add-proposed-group (group)
	      (let* ((i (tell (tell group 'get-leftmost-object) 'get-id-number))
		     (j (tell (tell group 'get-rightmost-object) 'get-id-number))
